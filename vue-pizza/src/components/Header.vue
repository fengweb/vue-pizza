<template>
	<header>
        ...
        <ul class="navbar-nav ml-auto">
        	<li><router-link v-show="!getIsLogin" to="/login" class="nav-link">登录</router-link></li>
            
            <li class="nav-link">
                {{ getCurrentUser }}
            </li>
            <li>
                <router-link :to="{name:'Login'}" v-show="getIsLogin" class="nav-link">[退出]</router-link>
            </li>

        	<li><router-link v-show="!getIsLogin" to="/register" class="nav-link">注册</router-link></li>
         </ul>
		</nav>
	</header>
</template>
